...
export class NotificationsComponent implements OnInit {
  ...
  redirectToProfile() {
    ...
    for (let idsofnoti of this.putview) {
      ...
      console.log(idsofnoti);
      this.http.get<NotificationsReceivers[]>('http://localhost:8188/notificationreceivers/Get?idNotificationReceiver=' + idsofnoti).subscribe(dataaux => {
        ...
        this.http.post<NotificationsReceivers>('http://localhost:8188/notificationreceivers/Update?idNotificationReceiver='
  ...
  getNotifications() {
    this.notviewed = [];
    this.putview = [];
    this.http.get<NotificationsReceivers[]>('http://localhost:8188/notificationreceivers/Get?idPlayerReceiverFK=' + this.session.playerSession.idplayer).subscribe(data => {
      ...
      for (let id of this.notificationids) {
        this.http.get<Notification[]>('http://localhost:8188/notifications/Get?ID_Notification=' + id).subscribe(data => {
