...
export class EventsComponent implements OnInit {
  ...
  allEvents() {
    this.http.get<Event[]>('http://localhost:8188/events/getAll').subscribe(data => {
      this.events = data;
    });
  }

  getAttendedEvents() {
    this.http.get<AttendedEvents[]>('http://localhost:8188/attendedEvents/Get?idPlayerFK=' + this.session.playerSession.idplayer).subscribe(data => {
      const obj = JSON.stringify(data);
      this.attendedEventsByPlayer = data;
      for (let aux of this.attendedEventsByPlayer) {
        this.http.get<Event[]>('http://localhost:8188/events/get?idEvent=' + aux.ideventFK).subscribe(res => {
  ...
  Save() {
    ...
    if (this.idOfEventsChecked != undefined) {
      for (let idEvent of this.idOfEventsChecked) {
        ...
        this.http.post<any>('http://localhost:8188/attendedEvents/Create?idEventsFK=' + id + '&idPlayerFK=' + idplayer,
          {
            id,
            idplayer
          }).subscribe(data => {
        ...
        var eventaux: Event;
        this.http.get<Event[]>('http://localhost:8188/events/get?idEvent=' + idEvent).subscribe(res => {
          const obj2 = JSON.stringify(res);
          eventaux = res[0];
          const idPlayer = this.session.playerSession.idplayer;
    ...
    if (this.idOfEventsUnchecked != undefined) {
      for (let idEvent of this.idOfEventsUnchecked) {
        const id = idEvent;
        const idplayer = this.session.playerSession.idplayer;
        this.http.post<any>('http://localhost:8188/attendedEvents/Delete?idEventsFK=' + id + '&idPlayerFK=' + idplayer,
          {
            id,
            idplayer
          }).subscribe(data => { });
