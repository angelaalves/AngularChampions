...
export class AddEventsComponent implements OnInit {
  ...
  onSubmit(form: NgForm){
    ...
    xhr.open('POST', 'http://localhost:8189/events/Create?eventName=' + name + '&eventDate=' + date +
      '&eventType=' + eventType , true);
    this.http.post<any>('http://localhost:8189/events/Create?eventName=' + name + '&eventDate=' + date +
    '&eventType=' + eventType,
      {
        name, 
        date,
