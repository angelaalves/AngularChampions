...
export class AddGuildListComponent implements OnInit {
  ...
  createGuild(form: NgForm){
    ...
    this.http.post('http://localhost:8188/guild/Create?guildName='+name+'&startDate='+formatDate(startDate, "yyyy-MM-dd","en-UK")+'&endDate='+formatDate(endDate, "yyyy-MM-dd","en-UK")+'&guildFlag=../../assets/AppImages/DefaultFlag.png&status=Active',{}).subscribe(response=>{
      this.http.get('http://localhost:8188/guildPlayers/createRecent?startDate='+formatDate(startDate, "yyyy-MM-dd","en-UK")+'&guildmaster='+this.guildmasterSelected.idplayer+'&players='+playerIds.toString()).subscribe()
    });
